...
| `system` 角色被上游丢弃 | 同一句格式约束分别放 `system` / `user`，各 3 次独立采样（**换不同主题**，避开缓存） | **两者都 3/3 生效 —— `system` 正常，不是被丢弃** |
...
> 这一条曾让整轮排查走偏 —— 最早两组「`system` 被丢弃」的对照实验，换主题重测后都是 3/3 生效。

**保留的一处防御性处理**：`lib/zhihu/client.ts` 的 `foldSystemIntoUser()` 把 `system` 内容
折叠进第一条 `user` 消息。**它不是必需的**（A/B 实测两种写法无差异），保留的理由是：
上游对 `system` 的遵从**不够稳定**（同一条请求时好时坏），把指令放进 `user` 是对这种不稳定的
低成本对冲；且实测对生成质量无副作用（回答长度与雷同度均无变化）。
折叠**之后**才计算缓存键，调用方仍可照常写 `role: "system"`。
