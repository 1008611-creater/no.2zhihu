...
> ## ⚠️ 时效快照（2026-09-16 补，供读者定位）
>
> **本页是 2026-09-14 的快照，正文保留原样不改** —— 它是当时真实的观测记录，
> 而下面列出的几项**已经过期**。核对方式与依据一并写在表里，可复现。
>
> | 本页写的 | 2026-09-16 的实际情况 | 依据 |
> |---|---|---|
> | 「6 位答主」（D5 / D7 / 第五节目录） | **16 位**（15 位真实蒸馏 + 1 位预置） | `lib/domain/personas/` 16 份 `.ts`；`real:true` 15 / `false` 1（`jiangxiaozhang`） |
> | D7「⬜ 阻塞：待提供 cookie」 | **已解除** —— 15 位已实跑抓取，共 448 条（14×30 + 采铜 28） | 各人格文件 `corpus.sampleSize` 可逐位核对 |
> | D6「如实标注『预置人格 · 未抓取全量回答』」 | 该标签**已于 2026-09-15 移除**（读起来像免责声明）；诚实性改由答主档案页正文承担 | `docs/submission.md` §答主人格从哪来；`corpusLabel()` 现在只在 `real:true` 时返回「基于公开片段提取 · N 条有效样本」 |
> | ③ 提交材料「④ 演示视频 🟡 待录制」 | 仓库里**没有视频产物**；本页与 `prerequisites.md` 都仍标「待录制」，未见状态更新（**是否有外部录制的成片不在此判断范围内**） | `git ls-tree -r main` 里 `.mp4/.mov/.webm` 命中 0；`docs/demo-video-script.md` 脚本就绪 |
> | 头部「截止 2026-09-15 10:00」 | **已过去**；当前处于**评审期（09-15 → 09-17，滚动评分）**，09-19 决赛路演 | 赛事官方时间线 |
>
> **写今日状态时请以 `docs/submission.md` 与 `docs/threads/` 为准**，不要以本页的 ✅/🟡 作为交付结论。
> 本页仍然有效的用途是：**逐项验收清单本身**（该验什么、怎么验）与**浏览器实测那一节的历史问题记录**。
...
用户授权后，在 https://zhihu.cauai.fun/ 点击首页示例「30 岁从大厂转行做独立开发，值得吗？」→ 选择答主 → 保留默认半佛仙人、贱贱、大猛 → 点击作答一次。

- 交互通过：页面从处理中进入「这一轮闭环完成」，显示 8/8 步、3 位答主分身、9 条来源、1 个缺口、3 篇直答生成；看山状态变为 celebrate。
- 来源展示通过：回答卡可见实际引用作者名和知乎 URL，例如小加科技资讯、Jackie Lee、超级大笨狼。仅验证页面展示，未逐篇核对上游原文或生成断言。
- 缺口展示通过：出现「提问者的具体条件没有被回答」与真人候选；关系图显示 22 节点、21 条关系。
- 未执行：邀请真人、互相回应、复制正文、外部发布、移动端与失败路径测试。当前结果不能等同完整产品验收。

本次发现的待修问题（页面证据，尚未定位代码原因）：

1. 归属表述冲突：页面称「正文只允许引用他对应的真实来源」，但半佛仙人卡引用小加科技资讯等作者，贱贱卡引用 Jackie Lee 等作者。需明确区分预置模拟人格与引用内容的真实作者，不可暗示选定答主本人作答或来源属于本人。
2. 证据相关性不足：独立开发问题中，贱贱的证据主要是「力学专业有多难学、就业前景如何」，大猛回答转向 CNC 加工厂。缺口卡按命中和赞同数推荐这些作者，尚不能证明其满足所需经历。
3. 回答完整性待查：半佛仙人正文停在第二点，贱贱停在「第三，大厂经历在独立开发里值多少？」；搬运正文也保留相同结尾。需查生成长度与截断机制。
4. 生成断言待核：大猛回答包含行业百分比及协会调研归因，页面未提供逐条对应证据；本次不得将这些数字视为已核实事实。

结论：单次核心交互成功；内容归属、相关性与完整性尚未通过。验证方式为浏览器可访问性树；本次无代码变更、无新增发布，主 skill 不适用（总纲 §3 #12）。
