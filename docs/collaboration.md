...
> 当时收敛为两条线程 —— **同一时间只有一个活跃的特性分支**，这一类问题从根上消失。
>
> **2026-09-15 晚更新（面对现实）。** 实际并行度已回到 **5 个开发 + 1 个审计**。
> 上面那条否决理由**依然成立**，同日晚间又实测到三例：
> 审计线程提的两个 PR 因「不知道别人已做同一件事」而白费；
> 开发线程基于过期 base 改 CI，差点回退别人 32 行；多个线程用陈旧的本地 clone 得出**相反**的结论
> （「脚本不在 main 上」，而它早在数小时前就已入库）。
>
> 所以这次**不是「再赌一次多线程」**，而是**给多线程补上它缺的那套机制**：
> [`threads/` 线程状态板](threads/README.md)（见第三节）—— 用**可读、可验证**的声明，
> 把协调成本压回去。**若机制压不住成本，仍应退回二线程。**
...
| 线程 | 职责 | 不做 |
|---|---|---|
| **开发线程（可多个）** | 改代码、自测、提 PR、**维护自己在 `docs/threads/` 的声明** | 不碰服务器、不部署、不合并 |
| **审计上线线程（唯一）** | 审计 PR、合并、线上复验、**抽查声明是否与实际相符** | 不写特性代码 |

**二线程时**，简化收益来自「同一时间只有一个开发线程在写」。
**多线程时**，收益不再自动成立 —— 必须由第三节的机制换回来。

---

## 三、线程状态板（多线程的必要机制，2026-09-15 新增）

规范见 [`threads/README.md`](threads/README.md)。要点：

- **一个线程一个文件**：`docs/threads/<线程名>.md`，随第一个 PR 一起提
- 声明内容：工作区 / 分支 / **正在改哪些文件** / 基线 sha / 状态 / 最后更新
- **开工前先读别人的** —— 比 `preflight.mjs` 早一步：那个只看**已提的 PR**，这里看**在做的**
- **超过 24 小时未更新 = 失效**，任何人可删（宁可没有，也不要留一份骗人的）
- **声明与实际不符，比没有声明更糟** —— 审计线程会抽查

**为什么必须走 git，而不是建一个本地共享目录**：本地目录没有历史（判断不了谁改的）、
换台机器就不通、无法做冲突检测，而且最终一定会像 `.tools/` 那样越堆越乱。
**分工**：权威状态 → 仓库 `docs/threads/`；临时产物与日志 → 本地工作区（不进仓库）。
