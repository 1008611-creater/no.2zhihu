...
lib/
  ...
  server/                       编排层（唯一同时接触知乎 IO 与领域规则）
    mirror.ts                   ★ 核心：问题 → 路由 → 证据 → 回答 → 缺口
  domain/                       纯业务逻辑（无 IO）
    types.ts                    领域模型
    skills.ts                   Skill 分身目录
    router.ts                   Human Router 打分
    gap.ts                      ★ 缺口识别
...
```
浏览器
  │  POST /api/mirror { question }
  ▼
route handler（薄，只做校验与转发）
  │  ① Zod 校验入参
  ▼
lib/server/mirror.ts（编排层，唯一同时接触 IO 与领域规则）
  │  ② routeQuestion(question)         ← lib/domain，纯函数，0 额度
  │  ③ 并行对每个分身：zhihuSearch()    ← lib/zhihu，带缓存
  │  ④ zhida(evidence → 该视角草稿)    ← 串行 + 退避重试（并发会触发 429）
  │  ⑤ findGaps(answers)               ← lib/domain，纯函数
  │  ⑥ 组装 MirrorQuestion
  ▼
JSON（不含任何凭证）
  │
浏览器渲染：分身卡 + 证据 + 缺口 + 看山状态
```
