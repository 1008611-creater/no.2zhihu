...
| 变体 | 行为 |
|---|---|
| `LogoMark` | 静止时两层严丝合缝（与静态 logo 完全一致）；hover / focus / tap 时分身向侧后方移 `x:8 y:-12 scale:1.045`，真人仅微退 `x:-10 y:4`，让「本体稳住、影子才有动作」成立 |
| `LogoMarkHero` | 挂载后自演一次「分离 → 归位」作开场，随后极缓慢循环呼吸。**只演一次是刻意的**——持续运动会占用合成线程并分散注意力 |

- 位移量刻意压到图形宽度的约 4%，品牌标识的首要职责是稳定可辨认。
- **暖白底砖 `#F7F6F3` 不可抠成透明**：黑头 `#151517` 与站点底色 `#07080f` 亮度仅差约 3%，
  抠掉后黑头会直接融进背景。保留底砖同时让动态版与静态图标是同一块「品牌砖」。
- 图层由 `scripts/split-logo-layers.py` 从 `public/logo.png` 拆分生成（纯 numpy 连通域 + 边缘软化）。
- `prefers-reduced-motion` 下直接降级为静态 `<img src="/logo.png">`，不渲染 SVG。
