...
| 阻塞 | 处置 |
|---|---|
| ~~刘看山官方素材包未下载成功~~ | **已解决**。素材包已由用户提供并逐字节核对为官方原文件，解压到 public/kanshan/（ASCII 改名），Kanshan 组件改为渲染官方素材。详见 [character-engine.md](character-engine.md) |
...
## 八之二、UI / 嵌套架构改造（2026-09-14 晚）

| 项 | 状态 |
|---|---|
| 看山改用官方素材，删除自绘 SVG 引擎 | 完成 |
| 新增 /personas 答主名册页 | 完成 |
| 新增 /personas/[handle] 答主档案页（四要素 + 蒸馏依据） | 完成 |
| 顶栏加「答主名册」，窄屏收成抽屉 | 完成 |
| 首页支持 ?persona=handle 预选答主（档案页 CTA 直达） | 完成 |
| 分身卡片加「查看完整人格档案」入口 | 完成 |
| 页面过渡（app/template.tsx）+ 首屏错峰入场 | 完成 |
| 焦点环、按钮按压、可点 chip 的交互反馈 | 完成 |
| 流程轨道 .rail 样式（供流程可视化复用） | 完成 |
