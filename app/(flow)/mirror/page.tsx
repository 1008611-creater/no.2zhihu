 ...
 *
 * 2026-09-15 两条主线重构：原先顶部的「分身发现」区块（DiscoverSection）
 * 已整体迁到 `/discover`（组件在 components/discover/PersonaRoster.tsx）。
 * 那一块属于另一条主线，长在这里会让「查看本场回答」的用户一进来先撞上一份
 * 全量名册。现在这一页只讲一件事：**这一场怎么了**。
 * 想看有哪些分身，走导航上的「分身发现」Tab。
 */
export default function MirrorPage() {
  ...
  // 还没有本场结果时，给两条明确的出口：回提问主线，或去另一条主线逛人。
  // 之前这里会在同一页里铺开全量答主名册 —— 那是 `/discover` 的职责。
  if (!mirror) {
    return (
      <div className="page-enter">
        <section style={{ paddingTop: 36 }}>
          ...
          <div style={{ marginTop: 22, display: "flex", gap: 10, flexWrap: "wrap" }}>
            <Link className="btn btn-primary" href="/">去提一个问题 →</Link>
            <Link className="btn btn-ghost" href="/discover">先看看有哪些分身 →</Link>
          </div>
        </section>

        <section className="section">
          <div className="notice notice-info">
            这里是提问流程的第二站：回答、互相回应、看山标出的缺口、搬运回知乎，都会落在这一页。
            想看已经跑完的讨论组，去
            <Link className="link" href="/square" style={{ margin: "0 4px" }}>虚拟广场</Link>
            挑一个载入工作台。
          </div>
        </section>
      </div>
